...
GetOptions( \%opt,      'nobad',        'nogood',       'noinfo',
            'nocolor',  'forcemem=i',   'forceswap=i',  'host=s',
            'socket=s', 'port=i',       'user=s',       'pass=s',
            'skipsize', 'checkversion', 'mysqladmin=s', 'help', );
...
sub get_all_vars
  {
    ...
    my @mysqlvarlist =
      `mysql $mysqllogin -Bse "SHOW /*!50000 GLOBAL */ VARIABLES;"`;
    ...
    my @mysqlstatlist =
      `mysql $mysqllogin -Bse "SHOW /*!50000 GLOBAL */ STATUS;"`;
...
sub security_recommendations
  {
    ...
    my @mysqlstatlist =
      `mysql $mysqllogin -Bse "SELECT CONCAT(user, '\@', host) FROM mysql.user WHERE password = '' OR password IS NULL;"`;
    if ( @mysqlstatlist )
      {
...
sub mysql_version_ge
  {
    ...
    return $mysqlvermajor > $maj
      || ( $mysqlvermajor == $maj && $mysqlverminor >= ( $min || 0 ) );
  }
...
sub check_storage_engines
  {
    ...
    if ( mysql_version_ge( 5 ) )
      {
        my @engineresults =
          `mysql $mysqllogin -Bse "SELECT ENGINE,SUPPORT FROM information_schema.ENGINES WHERE ENGINE NOT IN ('performance_schema','MyISAM','MERGE','MEMORY') ORDER BY ENGINE ASC"`;
...
# MySQL 5 servers can have table sizes calculated quickly from information schema
        my @templist =
          `mysql $mysqllogin -Bse "SELECT ENGINE,SUM(DATA_LENGTH),COUNT(ENGINE) FROM information_schema.TABLES WHERE TABLE_SCHEMA NOT IN ('information_schema','mysql') AND ENGINE IS NOT NULL GROUP BY ENGINE ORDER BY ENGINE ASC;"`;
        ...
        $fragtables =
          `mysql $mysqllogin -Bse "SELECT COUNT(TABLE_NAME) FROM information_schema.TABLES WHERE TABLE_SCHEMA NOT IN ('information_schema','mysql') AND Data_free > 0 AND NOT ENGINE='MEMORY';"`;
        chomp( $fragtables );
      }
    else
      {
        ...
        foreach my $db ( @dblist )
          {
            ...
            push( @tblist,
                  map { [ ( split )[ @ixs ] ] }
                    `mysql $mysqllogin -Bse "SHOW TABLE STATUS FROM \\\`$db\\\`"`
                );
          }
...
sub calculations
  {
    ...
    $mycalc{'total_possible_used_memory'} =
      $mycalc{'server_buffers'} + $mycalc{'total_per_thread_buffers'};
    $mycalc{'pct_physical_memory'} =
      int( ( $mycalc{'total_possible_used_memory'} * 100 ) / $physical_memory );
    ...
    if ( $doremote eq 0 and !mysql_version_ge( 5 ) )
    ...
    elsif ( mysql_version_ge( 5 ) )
      {
        $mycalc{'total_myisam_indexes'} =
          `mysql $mysqllogin -Bse "SELECT IFNULL(SUM(INDEX_LENGTH),0) FROM information_schema.TABLES WHERE TABLE_SCHEMA NOT IN ('information_schema') AND ENGINE = 'MyISAM';"`;
      }
    ...
    if ( mysql_version_ge( 4 ) )
      {
        ...
        if ( $myvar{'query_cache_size'} )
          {
            $mycalc{'pct_query_cache_used'} =
              sprintf( "%.1f",
                       100 - ( $mystat{'Qcache_free_memory'} /
                                 $myvar{'query_cache_size'}
                         ) * 100 );
          }
        ...
        if ( $mystat{'Table_locks_waited'} == 0 )
        ...
        else
          {
            $mycalc{'pct_table_locks_immediate'} =
              int( $mystat{'Table_locks_immediate'} * 100 / (
    ...
    # Thread cache
    $mycalc{'thread_cache_hit_rate'} = int( 100 -
...
sub mysql_stats
  {
    ...
    if (    $mycalc{'total_possible_used_memory'} > 2 * 1024 * 1024 * 1024
         && $arch eq 32 )
      {
        badprint
          "Allocating > 2GB RAM on 32-bit systems can cause system instability\n";
        badprint "Maximum possible memory usage: "
          . hr_bytes( $mycalc{'total_possible_used_memory'} )
          . " ($mycalc{'pct_physical_memory'}% of installed RAM)\n";
      }
